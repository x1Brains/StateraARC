...
// ⛔ 09-26 (owner: GLITCH card posted with NO image): X fetches the card image ONCE, gives up after a few seconds, and
// keeps the broken card for that post. Every step here had its own timeout but nothing bounded the SUM (list 6s x2 +
// price 3 RPCs x 3s + logo 3 x 2.5s = up to ~28s). `within` = a hard deadline that settles with a fallback, never throws.
export const within = (p, ms, fb = null) => Promise.race([Promise.resolve(p).catch(() => fb), new Promise((r) => setTimeout(() => r(fb), ms))]);
...
// Warm instances keep the list for 60s (it's 2.9MB / ~3,900 tokens), so back-to-back page + image renders don't refetch it.
let snapCache = { at: 0, byAddr: null };
export async function snapshotRow(origin, addr) {
  if (snapCache.byAddr && Date.now() - snapCache.at < 60000) return snapCache.byAddr.get(addr) || null;
  for (const u of [`${origin}/api/snapshot`, `${origin}/tokens-snapshot.json`]) {
    try {
      const snap = await fetch(u, { signal: AbortSignal.timeout(2000) }).then((r) => r.json());
      if (!snap || !Array.isArray(snap.tokens)) continue;
      const byAddr = new Map(snap.tokens.map((x) => [(x.address || '').toLowerCase(), x]));
      snapCache = { at: Date.now(), byAddr };
...
export async function chainRow(origin, addr) {
  ...
  for (const p of cands) {
    ...
    if (!t0) continue; // unknown orientation = unusable pool (a guessed side inverts the price)
    if (!best || usdc > best.usdc) best = { pool: p, usdc, usdcIsC0: ('0x' + t0.slice(-40)).toLowerCase() === USDC, s0, tok: tb ? Number(BigInt(tb)) / 10 ** decimals : 0 };
  }
...
// Same hard rules as the site (src/lib/arc.ts sanitizeToken): a card never shows an impossible number.
function sane(t) {
  if (!t) return t;
  const r = { ...t }; const fin = (v) => (typeof v === 'number' && isFinite(v) ? v : null);
  r.price = fin(r.price); r.liq = fin(r.liq); r.mcap = fin(r.mcap); r.volume24h = fin(r.volume24h);
  if (r.price != null && (r.price <= 0 || r.price >= 1e6)) { r.price = r.liq = r.mcap = r.volume24h = null; r.change24h = null; }
  if (!r.isEcosystem) { if (r.mcap > 5e8) r.mcap = null; if (r.liq > 5e7 || (r.mcap > 0 && r.liq > r.mcap * 1.5)) r.liq = null; if (r.volume24h > 5e7 || (r.liq > 0 && r.volume24h > r.liq * 20)) r.volume24h = null; }
  return r;
}
export async function liveToken(origin, addr) { return sane(await liveTokenRaw(origin, addr)); }
async function liveTokenRaw(origin, addr) {
  let t = await within(snapshotRow(origin, addr), 4000);
  if (!t) return within(chainRow(origin, addr), 3000); // not listed → built from chain (price already live)
  const live = await within(chainPrice(t), 1500); // slow RPC → the list's price (≤30 min old) rather than no card
